    ...
    <div>
        ...
        <div class="wrapper">
			<div class="login" >
				...
				<p>{{addDel(billTwo.type)}}<em>{{Fixed(billTwo.currency_num)}}</em></p>
			</div>	
			<ul class="pualic_detail_list"> 
                ...
                <li>
                    <h3>交易单价</h3>
                    <span>{{Fixed(billTwo.unit_price)}}</span>
                </li>                
                <li>
                    <h3>交易类型</h3>
                    <span>{{tradeType(billTwo.type)}}</span>
                </li>
                <li>
                    <h3>交易状态</h3>
                ...
                <li>
                 ...
                 <li>
                    ...
                    <span class="omit" id="omit">{{billTwo.trade_address}}</span>  
                </li>   
                <li  v-if="billTwo.type == '20' ">
                    <h3>交易时间</h3>
                    <span>{{billDetail.create_time}}</span>
                </li>                
                 <li v-if="billTwo.type == '10' ">
                    <h3>下单时间</h3>
                    <span>{{billDetail.create_time}}</span>
                </li>  
                 <li v-if="billTwo.type == '10' ">
            ...
            <div class="footer" v-if="(billTwo.type == '10' && billDetail.status == '41') ||  (billTwo.type == '20' && billDetail.status == '40') ">
            	<p v-if="billTwo.type == '10' && billDetail.status == '41'">待确认收货</p>
            	<p v-else>待确认发货</p>
            	<div class="foot-but">
            		<!--<div class="cancel">取消订单</div>-->
            		<div class="pay payone" v-if="billTwo.type == '10' && billDetail.status == '41'" v-tap="{methods: collect}">确认收货</div>
...
export default {
	...
	mounted(){
		...
		isPlus.then(() => {
			...
			console.log('this.billTwo' + JSON.stringify(this.self.billDetail));
			this.billTwo = this.self.billDetail;
			this.billDetail = this.self.billDetail.get_purchase_record;
			this.$nextTick(() => {
				if(this.self.billDetail.type == '10'){
					this.title = this.self.billDetail.get_currency_info.character + '买入详情';
				} else{
					this.title = this.self.billDetail.get_currency_info.character + '卖出详情';
				}
				this.character = this.self.billDetail.get_currency_info.character;					
			})
		})	
	},	
	methods:{
		copys(){            // 点击复制
			var bank = (this.billTwo.trade_address).replace(/\s/g, "");
